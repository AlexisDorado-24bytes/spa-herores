...
export class HerosService {
  constructor() {
    console.log('Servicio Heros Iniciado');
  }

  private herosData: Heros[] = [
    {
      nombre: 'Aquaman',
      bio:
  ...
  heroBuscardo(expresion:string){
    let herosEncontrado:Heros[] = [];
    
    expresion = expresion.toLowerCase();
    for (let hero of this.herosData) {
    
      
      if(hero.nombre.toLowerCase().indexOf(expresion.toLowerCase()) >= 0 ){
        
        herosEncontrado.push(hero);
      }
    }
    return herosEncontrado;
  }

  myHeroGet(inr: string) {
    var numHeros = this.herosData.length;
    for (let index = 0; index < this.herosData.length; index++) {
      const element = this.herosData[index].nombre.toLowerCase();
      if(element.indexOf(inr.toLowerCase()) >= 0 ){
        return this.herosData[index];
